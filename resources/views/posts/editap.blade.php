            ...
            <div class="card-body">
                <h5 class="card-title">
                    ...
                    <form action="{{route('apartments.update', $apartment->id)}}" method="post">
                        @csrf
                        @method('PUT')
                        Apartment name: <input type ="text" name="title" value="{{$apartment->title}}"> <br>
                        Stars: <textarea name="content" cols="30" rows="1">{{$apartment->content}}</textarea>
                        <select name="category_id">
                            @foreach($cats as $cat)
                                <option value="{{$cat->id}}" @if($cat->id == $apartment ->category_id) selected @endif>{{$cat->name}}</option>
                            @endforeach
                        </select>
